...
- Relay can bind to a configurable host with `NUDGE_RELAY_HOST`, defaulting to `0.0.0.0` for local LAN and tunnel testing. `npm run relay:local` starts the hardened development relay and prints simulator/LAN URLs. `docs/local-first-run.md` defines the current first-version path: development-machine relay, simulator smoke, same-network phone such as `http://10.10.10.xxx:8787`, or Cloudflare Tunnel URL before TestFlight/hosted deployment work.
