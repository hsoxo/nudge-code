...
- Device key rotation is implemented at the relay API/auth layer with current-key-signed rotation transcripts, nonce replay rejection, public-key validation, and websocket attach coverage after rotation. iOS rotation client support updates the Keychain identity and local binding only after relay acceptance. Daemon rotation and scheduled rotation UX remain future hardening.
