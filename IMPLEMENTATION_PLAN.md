...
- Mouse tab selection, rename inside the TUI, daemon-produced embedded ANSI render frames, and CLI-side incremental render diffing are implemented.
...
- Relay device key rotation is available through `POST /api/devices/rotate-key`; a device signs the rotation transcript with its current Ed25519 key, the relay validates the new key, rejects replayed rotation nonces, updates the stored public key, and records a metadata-only `device_key_rotated` audit event. Relay smoke verifies the old key stops authorizing websocket attaches and the new key works. Daemon and iOS clients can rotate their local identities by generating candidate keys, signing the relay transcript with the current key, committing only after relay acceptance, and persisting the rotated binding public key.
...
- Relay has `NUDGE_RELAY_HOSTED_MODE=1` as a single-process hosted preset that requires signed websockets, one-shot websocket challenges, E2E relay payloads, and disables legacy HTTP message endpoints. `/readyz` exposes the hosted-mode state, config flags, runtime counts, and warnings for weak hosted settings such as missing persistence, unsigned websockets, missing websocket challenges, legacy HTTP messages, and non-required E2E envelopes.
- Relay can bind to a configurable host with `NUDGE_RELAY_HOST`, defaulting to `0.0.0.0` for local LAN and tunnel testing. `NUDGE_RELAY_URL` is the canonical advertised relay URL for the packaged CLI and relay, and can point at `127.0.0.1`, a `10.10.10.xxx` LAN URL, or a Cloudflare Tunnel URL. `npm run relay:local` starts the hardened development relay and prints simulator/LAN URLs. `npm run bind:local` checks relay health and starts the computer-side wait/confirm flow. `docs/local-first-run.md` defines the current first-version path before TestFlight/hosted deployment work.
...
- spawned-process smoke self-starts a hardened relay and daemon, binds a simulated phone through the real CLI flow, performs E2E handshake, sends encrypted terminal input, receives encrypted daemon response/live output, reconnects the simulated phone, and replays encrypted terminal output.
...
- Native iOS QR scanning, phone signed websocket URLs, daemon signed websocket URLs, participant public key caching, live relay E2E encryption wiring, spawned relay/daemon/simulated-phone E2E smoke coverage, in-process iOS relay-session coverage for encrypted reconnect replay, and a gated iOS simulator smoke for claiming a real relay binding while the real daemon-side CLI waits and confirms are implemented. The same gated smoke now opens the native relay session and verifies session state, terminal output replay, terminal input, and daemon-pushed live output against the real daemon. Physical-device mobile app coverage against a hosted relay remains open Phase 5/relay work.
...
- The scaffold includes machine list, tab strip, terminal preview WebView with bundled xterm.js assets, relay-backed phone/computer width control, pairing URL parsing, `nudge://pair` deep-link handling, pending binding UI, camera QR scanning, SwiftProtobuf-generated protocol types, Keychain-backed phone signing identity, relay claim client with persisted machine binding metadata storage, one-shot binding status refresh, mobile-initiated binding revoke from the machine list, long-lived mobile websocket sync for session state/snapshots/input responses, phone profile reporting, automatic relay session reconnect with stale-state banner and app background/foreground session restart handling, terminal snapshot fetch, byte-safe replayed terminal output tail, throttled ephemeral daemon-pushed live terminal byte updates, terminal input over relay from the shortcut keyboard/composer and direct xterm raw keystrokes, and adaptive shortcut keyboard model/tests for Claude/Codex approval/waiting states.
- iOS and daemon websocket signatures are implemented; E2E handshake/encrypted relay requests are implemented in RelayClient. The long-lived mobile relay session now accepts daemon-pushed agent status updates without forcing a terminal snapshot, so shortcut keyboards can react to Claude/Codex state changes. The iOS terminal model preserves terminal output as base64 bytes, distinguishes reconnect replay from live incremental output, and keeps a bounded replay buffer for WebView rebuilds. In-process iOS relay-session tests cover encrypted terminal input, daemon-pushed live output, and reconnect replay. `scripts/smoke-ios-relay-claim.sh` covers a real hosted-mode relay, real daemon bind wait/confirm flow, iOS simulator RelayClient claim, native session state/output replay, terminal input, and live terminal output from the daemon. Physical-device mobile app terminal-session coverage remains open.
...
- iOS simulator can claim a real relay pairing while daemon CLI waits and confirms.
- iOS simulator can open a native relay session to the real daemon and verify tab state, terminal replay, terminal input, and live output.
...
- `scripts/install.sh` detects macOS/Linux architecture, downloads a release tarball, verifies a `.sha256` file unless `NUDGE_SKIP_CHECKSUM=1`, installs `nudge` into the configured user bin directory, supports local release URL overrides for smoke tests, and can verify signed artifacts with an inline public key, local public key file, or public key URL.
...
- `nudge update` re-runs the native installer from `https://nudgecode.dev/install.sh` by default, supports dry-run, version override, install-dir override, checksum skip forwarding, local installer paths for tests, and forwards signature requirement plus public-key file/URL verification settings.
- GitHub Actions release workflow scaffolding builds and uploads macOS/Linux release artifacts with `.sha256` files and publishes them on tag pushes. The packager can additionally emit RSA/SHA-256 `.sig` files when `NUDGE_SIGNING_KEY` is set, the installer can require and verify those signatures with a supplied public key, and CI can sign artifacts when `NUDGE_RELEASE_SIGNING_KEY_PEM` is configured. `scripts/prepare-install-site.sh` assembles a deployable static hosting bundle with `install.sh`, versioned releases, `latest` release copies, signatures, checksums, and `releases/nudge-release-public.pem` when `NUDGE_RELEASE_PUBLIC_KEY_PEM` is configured. The release workflow publishes that bundle as `nudge-install-site.tar.gz`. The hosted install/update flow is documented in `docs/install.md`; actual `nudgecode.dev` static hosting remains open.
...
- Device key rotation is implemented at the relay API/auth layer with current-key-signed rotation transcripts, nonce replay rejection, public-key validation, and websocket attach coverage after rotation. Daemon and iOS rotation client support update local identity storage and local binding state only after relay acceptance. Scheduled rotation UX remains future hardening.
...
- self-starting spawned-process relay + daemon + simulated phone integration test for E2E handshake, encrypted terminal input, encrypted daemon response, and encrypted live terminal output
