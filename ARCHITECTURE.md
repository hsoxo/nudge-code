...
- device key rotation uses a current-key-signed relay transcript over `POST /api/devices/rotate-key`; the relay validates the new Ed25519 public key, rejects replayed rotation nonces, and updates the stored device public key. The daemon and iOS both generate candidate identities, sign rotation transcripts with the current key, commit the new key only after relay acceptance, and update the local binding public key.
...
- scheduled device key rotation UX and managed storage-backed rotation nonce sharing
