...
- **Progress:**
  ...
  - Code review (critic + Codex) of Phases 3/4.2/4.3 → **both CLEAN** after fixes. Caught a HIGH the mock-based unit test missed: `set_focused_tab` had been sent correlated as `.sessionState`, so the daemon's `{accepted:true}` ack decoded to an empty session state and **wiped all tabs on every tab switch** — fixed (fire-and-forget ack) + wire-level regression test. Also fixed an idle-refocus staleness regression (re-baseline the focused tab on switch) and two perf/ordering minors.
