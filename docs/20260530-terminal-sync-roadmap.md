...
- **Date:** 2026-05-30
- **Status:** Phases 0–2 shipped + reviewed CLEAN; Phase 3 (3.1/3.2) shipped; Phase 4 (4.2 buffers incl. JS ring, 4.3 focused-tab) shipped; 4.1 binary-frames: **Level A shipped** (binary delta plaintext, daemon+phone; relay untouched), Level B scoped (`docs/20260531-phase4.1-binary-frames-scope.md`); Phase 5 spike done (2026-05-31)
- **Progress:**
  ...
  - Phase 2 complete (full-state snapshots) — 2.1 `state_frame()` `d209115`, 2.2/2.3 daemon+iOS wiring `cc0c286`. Scrollback (M1) deferred — **not** a vt100 API limit (it exposes `Parser::new(rows,cols,N)` + `Screen::set_scrollback`/`scrollback()`): post-attach history already survives via the phone's bounded replay buffer + xterm.js, so the only real gap is *pre-attach* history, and emitting that is a fiddly snapshot wire-format change best landed with on-device verification. **Pending on-device verification** (daemon/relay stack down this session) — the `state_frame()` round-trip is vt100-tested, not yet xterm.js-tested.
  ...
  - Phase 4 — 4.2 buffer efficiency (Data-backed replay buffer) `563925b`; 4.3 focused-tab priority `cbd6cf0`. **4.1 binary WS frames — Level A shipped, Level B scoped** (`docs/20260531-phase4.1-binary-frames-scope.md`): the relay *is* in this repo (`packages/relay`, `@nudge/relay`) — the earlier "not in this repo" note was **wrong**. 4.1 is implementable here; the real gate is on-device verification (stack down). Scope splits it into **Level A** (binary delta *plaintext*; daemon+phone only, relay untouched since it never sees plaintext; ~1.78N→~1.33N) and **Level B** (WS binary *frames*; +relay; ~1.0N but the relay's auth/routing/E2E-enforcement core must be replicated on the binary path). JS-side ring buffer (part of 4.2) ✅ shipped (see 4.2 below).
...
| **L2** | LOW | iOS re-base64 of 64KB/delta (O(n²)); JS per-byte buffers (✅ iOS Data-backed buffer Phase 4.2 `563925b`; ✅ JS `Uint8Array` ring buffer Phase 4.2) | `AppModel.swift:831`, `index.html:238` |
...
**3.3 Reflow (deferred, longer-term)** — the *visible screen* already reflows: `resize` replays `contents_formatted`, and vt100 emits soft-wrapped rows *without* a line break (`grid.rs:206`, gated on `row_wrapped`), so the text re-wraps at the new width when replayed into the resized parser. Only *scrollback* reflow is missing — moot while scrollback is not retained (M1). A reflow-aware grid (or a newer `vt100` resize API) would be needed for history reflow; the post-resize re-baseline covers the common case meanwhile.
...
### Phase 4 — Transport + buffer efficiency (effort: M) — 4.2/4.3 shipped; 4.1 Level A shipped, Level B scoped

**Goal:** cut bandwidth/CPU under bursts and for multi-tab.
**Findings:** L1, L2, M4.

**4.1 Binary frames (L1) — Level A ✅ SHIPPED, Level B 📐 SCOPED** (`docs/20260531-phase4.1-binary-frames-scope.md`). The earlier "BLOCKED — relay not in this repo" note was **wrong**: the relay is in this repo at `packages/relay` (`@nudge/relay`, Node/TS; `docs/relay-hosted.md:5`). Terminal bytes are base64'd **twice** (`bytesBase64` inside the inner JSON, then `ciphertextBase64` after E2E encrypt → ~1.78× amplification + ~300–400B fixed JSON/frame). Two levers: **Level A** — encrypt a binary delta *plaintext* (relay never sees plaintext, so daemon+phone only, no relay change; ~1.78N→~1.33N; needs capability negotiation); **Level B** — send the ciphertext as a WS *binary* frame (daemon+relay+phone; ~1.0N + drops the per-frame JSON, but the relay's route-auth/E2E-enforcement core must be replicated on the binary path). Level A is shipped (daemon+phone, capability-negotiated, snapshots stay JSON; offline-tested — Rust + Swift round-trip/negotiation; on-device burst/render verification pending, stack down). Level B is the follow-up, justified only if a device measurement shows per-frame JSON overhead still hurts small-frame latency.
**4.2 iOS/JS buffers (L2) — ✅ iOS + JS shipped.** iOS (`563925b`): `TerminalTab.replayOutputData: Data` backs a computed `replayOutputBase64`; the live append decodes only the delta (no whole-buffer re-encode). JS (`index.html`): `createByteRing` — a `Uint8Array`-backed bounded FIFO replacing the old `Array` + `splice(0,…)` (which shifted the whole buffer on every delta once full → O(n)/delta, O(n²)/session). Amortized-O(1) append (compacts at most once per `limit` bytes); `bytes()` returns a *copy*, since xterm retains the buffer for async processing. Fuzz-tested against a naive reference across buffer sizes + copy-safety + FIFO-drop-oldest.
...
**Risk:** medium — binary framing is the riskiest part: Level B touches the relay's route-auth/E2E-enforcement core (`packages/relay`), which must be replicated on the binary path or it becomes a bypass; Level A (binary plaintext) avoids the relay entirely. Can ship 4.2/4.3 independently of 4.1.
...
2. **vt100 0.16 capabilities (Phase 2):** ✅ resolved — `vt100 0.16.2` `Screen` exposes all needed modes (`alternate_screen`/`application_cursor`/`application_keypad`/`bracketed_paste`/`hide_cursor`/`mouse_protocol_mode`/`mouse_protocol_encoding`). Scrollback *is* accessible (`Parser::new` scrollback_len + `Screen::set_scrollback`/`scrollback()`); M1 stays deferred not for lack of API but because post-attach history already survives via the phone replay buffer, and emitting pre-attach history is an unverified-on-device wire change.
3. **Scrollback depth (M1):** target rows (e.g. match xterm's 2000) vs. memory on the daemon.
4. **Binary frames (Phase 4.1):** ✅ scoped (`docs/20260531-phase4.1-binary-frames-scope.md`). Level A (binary *plaintext*) needs **no** relay wire change; Level B (binary *frames*) does. Open sub-decision: Level A only (~1.33N, no relay change) vs. also Level B (~1.0N + small-frame win, relay change). Ties to #1 (capability marker — required for Level A negotiation).
