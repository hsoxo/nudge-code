...
- **Progress:**
  ...
  - Code review (critic + Codex) fixes — C1 stuck-awaiting-snapshot freeze `7894c48`; `state_frame()` cursor + canonical-mode restoration `cdb505e`; flush-interval clamp `5e18d46`. Accepted/deferred: drop-deltas-while-awaiting (full-screen replace on gap, no data loss), per-delta base64 re-decode (Phase 4.2), origin-mode/scroll-region/charset not restored (vt100 limitation).
