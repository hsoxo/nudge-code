...
- The grid's `total_bytes` (`lib.rs:39`, advanced in `process`, `lib.rs:53`) stays the **frame-sequence axis** *for the 5a.1 frame-aware payload*. A frame is stamped with the `total_bytes` value it was rendered at, read under the grid lock — identical to how `terminal_snapshot` reads `(snapshot(), state_frame())` coherently today (`lib.rs:1907-1915`). This works only because 5a.1 changes the phone's check to `baseOffset == nextOffset` (below). The *interim* 5a.0 prototype — which reuses the existing delta payload and the unchanged phone's `next += byteCount` arithmetic — must instead put **both deltas and snapshots on a cumulative sent-byte axis** (see the 5a.0 caveat in §4); do not mix the two axes.
...
- Net: (a) is expected to win on bursty output and on idle (an idle screen diffs to empty), and is worse in the pathological case of a full-screen repaint every tick — which the full-frame cap bounds. This is a *hypothesis to confirm in 5a.0*, not a measured result: the worst-case bytes-per-cell for a full frame on a wide tab (open question §5.2) and the per-flush `contents_diff` CPU (§5.1) both need real numbers before "(a) is better" is established. The L1/L2 base64 inefficiencies (Phase 4) apply equally to both models and are orthogonal.
...
- **5a.0 (S) — Prototype/measure, daemon-only.** Add a daemon-side `RenderCache` (copy from `nudge-cli/src/main.rs:1334-1360`) and, behind an env flag (e.g. `NUDGE_RENDER_FRAMES=1`), have `flush_pending_terminal_outputs` emit `render_frame`/`contents_diff` bytes instead of raw PTY bytes. **Offset-axis caveat (review):** the phone is *not* literally unchanged-and-correct if frames go through the delta payload as-is — `applyTerminalDelta` advances `nextOffset` by the payload's decoded byte length and gap-checks `offset == nextOffset`, but the daemon stamps a delta's `offset` from the **input** `total_bytes` axis (`accumulate_terminal_change`), which is unrelated to a *diff*'s byte length (and an empty visual diff has length 0 while `total_bytes` still advances). Shipped naively, every frame after the first reads as a gap → forces a snapshot → **full-frame-per-flush thrash that invalidates the very bandwidth number 5a.0 exists to produce.** Two honest options: **(a)** stamp **both** frame deltas **and** the full-frame snapshots on a single cumulative **sent-byte** axis (the running sum of emitted frame/diff bytes) instead of `total_bytes` — then the phone's `offset == next` / `next += len` delta arithmetic holds AND the snapshot it adopts (`nextOffset = snapshot.offset`) sits on the same axis the next delta is compared against, so a full-frame resync doesn't immediately re-gap. (Both halves matter: if only deltas move to the sent-byte axis while a snapshot keeps the `total_bytes` offset, the phone adopts the snapshot offset then gaps on the next sent-byte delta → thrash.) The diff bytes append correctly via `term.write`; the phone stays unchanged, the daemon just changes the offset *source* for frame mode. NB this is the *interim* 5a.0 axis — the 5a.1 frame-aware payload (§2.2) instead carries `{baseOffset, atOffset}` on the render-time `total_bytes` axis, valid because the phone's arithmetic changes then. **(b)** ship each frame on the **snapshot** payload (the phone applies it as a full replace via `resetAndReplay`, no offset arithmetic) — simpler, but measures *full-frame* not *diff* bandwidth. **Recommend (a)** for a true diff measurement. Either way it's daemon-only + reversible. **This is the recommended next step.**
...
2. Behind `NUDGE_RENDER_FRAMES=1`, in `flush_pending_terminal_outputs` (`lib.rs:3217`): instead of shipping `pending.data` raw, render the tab's grid (`render_frame` for a full frame, `contents_diff(prev)` for an incremental) and ship *those* bytes — but stamp **both the diff deltas and the full-frame snapshots** on a single **cumulative sent-byte axis** (running total of emitted frame bytes), NOT the input `total_bytes`, so the phone's `offset == nextOffset` / `next += len` delta arithmetic AND its `nextOffset = snapshot.offset` adoption stay on the same axis (option (a) above — mixing the two axes re-gaps after every full frame). Alternatively ship every frame on the snapshot payload (full replace, no offset math) to measure full-frame cost. Do NOT ship diffs through the delta payload with the input-offset axis — it gaps every frame.
