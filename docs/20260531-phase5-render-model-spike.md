...
- **5a.0 (S) — Prototype/measure, no protocol change.** Add a daemon-side `RenderCache` (copy from `nudge-cli/src/main.rs:1334-1360`) and, behind an env flag (e.g. `NUDGE_RENDER_FRAMES=1`), have `flush_pending_terminal_outputs` emit `render_frame`/`contents_diff` bytes through the **existing** delta/snapshot payloads (`relay_live_terminal_output_payload`/`..._snapshot_payload`, `lib.rs:3316/3288`). The phone needs *zero* changes — diff bytes are just escape bytes into `term.write`. This proves correctness on-device and lets us measure bandwidth (§2.4) before committing to any wire change. **This is the recommended next step.**
...
| Flag-gated frame emission through existing payloads (5a.0) | **S** | No wire change; reversible; on-device-measurable. |
...
2. Behind `NUDGE_RENDER_FRAMES=1`, in `flush_pending_terminal_outputs` (`lib.rs:3217`): instead of shipping `pending.data` raw, render the tab's grid (`render_frame` for a full frame, `contents_diff(prev)` for an incremental) and ship *those* bytes through the unchanged `relay_live_terminal_output_payload`/`..._snapshot_payload`. The phone needs no change — they are escape bytes.
